...
function checkRobotsAndSitemap() {
  ...
  if (fs.existsSync(robotsPath)) {
    ...
    if (content.includes('sitemap')) {
      addResult('passed', 'Robots', '✓ Next.js 动态 robots.ts 存在且包含 Sitemap 引用')
    } else {
      addResult('warnings', 'Robots', '⚠ robots.ts 缺少 Sitemap 引用')
    }
  } else {
    addResult('errors', 'Robots', '✗ robots.txt / robots.ts 不存在')
  }
  ...
  if (fs.existsSync(sitemapPath)) {
    ...
    addResult('passed', 'Sitemap', '✓ Next.js 动态 sitemap.ts 存在')
  } else {
...
function checkImages() {
  ...
  } else if (fs.existsSync(heroImagePath)) {
    const stats = fs.statSync(heroImagePath)
    const sizeKB = (stats.size / 1024).toFixed(2)
    addResult('passed', 'Images', `✓ hero.webp 存在并用于 OpenGraph (${sizeKB} KB)`)
  } else {
    addResult('errors', 'Images', '✗ og-image.jpg / images/hero.webp 不存在')
  }
...
function checkStructuredData() {
  ...
  const content = structurePaths
    .filter(filePath => fs.existsSync(filePath))
    .map(filePath => fs.readFileSync(filePath, 'utf-8'))
    .join('\n')

  // 检查是否有 Organization schema
  if (content.includes('Organization') && content.includes('WebSite') && content.includes('SearchAction')) {
    addResult('passed', 'Structured', '✓ 包含 WebSite / Organization / SearchAction 结构化数据')
  } else {
    addResult('warnings', 'Structured', '⚠ 未找到完整结构化数据（建议包含 WebSite / Organization / SearchAction）')
  }
}
